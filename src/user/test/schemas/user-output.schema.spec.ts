...
import { UserOutputSchemaHelper } from '../helpers/schemas/user-output.schema.helper';
...
describe('User output schema', () => {
  it('Should transform plain to user output schema', () => {
    const user = UserOutputSchemaHelper.createPlain();
  ...
  it('Should transform user output schema to plain', () => {
    const userSchema = UserOutputSchemaHelper.createSchema();
